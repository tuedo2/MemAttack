...
def prepare_model_performance(shadow_model, shadow_train_loader, shadow_test_loader,
    ...
    def _softmax_by_row(logits, T = 1.0):
    ...
    def _model_predictions(model, dataloader):
        ...
        for (inputs, labels) in dataloader:
            ...
            return_outputs.append(_softmax_by_row(outputs.data.cpu().numpy()) )
    ...
    return shadow_train_performance, shadow_test_performance, target_train_performance
...
def get_risk_scores(dset, test, net_type='VGG'):
    perm = torch.randperm(len(test))
    tr =  perm[:len(perm)//2]
    te = perm[len(perm)//2:]
    shadow_tr = torch.utils.data.Subset(test, tr)
    shadow_te = torch.utils.data.Subset(test, te)

    shadow_tr_loader = torch.utils.data.DataLoader(shadow_tr, batch_size=512, shuffle=False)
    shadow_te_loader = torch.utils.data.DataLoader(shadow_te, batch_size=512, shuffle=False)
    trainloader = torch.utils.data.DataLoader(dset, batch_size=512, shuffle=False)

    if net_type == "VGG":
        shadow_net = full_train_VGG11(shadow_tr, 10)
        target_net = full_train_VGG11(dset, 10)
    elif net_type == "Resnet":
        shadow_net = full_train_resnet(shadow_tr, 10)
        target_net = full_train_resnet(dset, 10)
    elif net_type == "Mobile":
        shadow_net = full_train_mobilenet(shadow_tr, 10)
        target_net = full_train_mobilenet(dset, 10)

    shadow_tr_perf, shadow_te_perf, target_perf = prepare_model_performance(shadow_net, shadow_tr_loader, shadow_te_loader, target_net, trainloader)
    shadow_tr_m_entr = m_entr_comp(shadow_tr_perf[0], shadow_tr_perf[1])
    shadow_te_m_entr = m_entr_comp(shadow_te_perf[0], shadow_te_perf[1])
    target_m_entr = m_entr_comp(target_perf[0], target_perf[1])

    scores = calculate_risk_score(shadow_tr_m_entr, shadow_te_m_entr, shadow_tr_perf[1], shadow_te_perf[1], target_m_entr, target_perf[1])
    return scores

def get_risk_scores_MNIST(dset, test, net_type='VGG'):
    perm = torch.randperm(len(test))
    tr =  perm[:len(perm)//2]
    te = perm[len(perm)//2:]
    shadow_tr = torch.utils.data.Subset(test, tr)
    ...
    if net_type == "VGG":
        shadow_net = full_train_VGG11_MNIST(shadow_tr, 5)
        target_net = full_train_VGG11_MNIST(dset, 5)
    elif net_type == "Resnet":
        shadow_net = full_train_resnet_MNIST(shadow_tr, 5)
        target_net = full_train_resnet_MNIST(dset, 5)
    elif net_type == "Mobile":
        shadow_net = full_train_mobilenet_MNIST(shadow_tr, 5)
        target_net = full_train_mobilenet_MNIST(dset, 5)
